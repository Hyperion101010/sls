...
impl Backend {
    ...
    /*
    fn construct_builtins(bin: sema::ast::Builtin) -> String {
        let msg = format!("built-ins");
        match bin {
            PayableSend => {
        ...
        match stmt {
            Statement::VariableDecl(_locs, _, _param, expr) => {
                if let Some(exp) = expr {
                    Backend::construct_expr(exp, lookup_tbl, symtab,fnc_map, ns);
                }
                let mut msg = (_param.ty).to_string(ns);
            ...
            Statement::If(_locs, _, expr, stat1, stat2) => {
                ...
                for st1 in stat1 {
                    Backend::construct_stmt(st1, lookup_tbl, symtab,fnc_map, ns);
                }
                for st2 in stat2 {
                    Backend::construct_stmt(st2, lookup_tbl, symtab,fnc_map, ns);
                }
            }
            ...
            Statement::For {
                ...
                for stat in init {
                    Backend::construct_stmt(stat, lookup_tbl, symtab,fnc_map, ns);
                }
                for stat in next {
                    Backend::construct_stmt(stat, lookup_tbl, symtab,fnc_map, ns);
                }
            ...
            Statement::Return(_locs, expr) => {
                for expp in expr {
                    Backend::construct_expr(expp, lookup_tbl, symtab,fnc_map, ns);
                }
            }
            Statement::Emit {
                loc,
                ...
                let fnc_ky = format!("{}{}", &ns.events[*event_no].name, &ns.events[*event_no].signature);

                if fnc_map.contains_key(&fnc_ky) {
                    match fnc_map.get(&fnc_ky) {
                        Some(ans) => {
                                let nm = format!("{}", ans[..].to_string());
                                lookup_tbl.push((loc.1 as u64, (loc.1 + &ns.events[*event_no].name.len()) as u64, nm));
                        }
                        _ => {}
                    }
                }
                else {
                    let msg2 = format!("emit not found for {}", fnc_ky);
                    lookup_tbl.push((loc.1 as u64, (loc.1 + &ns.events[*event_no].name.len()) as u64, msg2));
                }
                for arg in args {
            ...
            Statement::TryCatch {
                ...
                for vecstmt in catch_stmt {
                    Backend::construct_stmt(vecstmt, lookup_tbl, symtab,fnc_map, ns);
                }
                ...
                if let Some(okstmt) = error {
                        for stmts in &okstmt.2 {
                            Backend::construct_stmt( &stmts, lookup_tbl, symtab,fnc_map, ns);
                        }
                    }
                }
    ...
    fn construct_expr(
        ...
        match expr {
            ...
            Add(_locs, _typ, expr1, expr2) => {
                Backend::construct_expr(expr1, lookup_tbl, symtab,fnc_map, ns);
                Backend::construct_expr(expr2, lookup_tbl, symtab,fnc_map, ns);
            }
            ...
            Ternary(_locs, _typ, expr1, expr2, expr3) => {
                Backend::construct_expr(expr1, lookup_tbl, symtab,fnc_map, ns);
                Backend::construct_expr(expr2, lookup_tbl, symtab,fnc_map, ns);
                Backend::construct_expr(expr3, lookup_tbl, symtab,fnc_map, ns);
            }
            ...
            ExternalFunctionCall {
                ...
                let fnc_nam = &ns.contracts[*contract_no].functions[*function_no].name;
                let n_msg = format!("external func call for {}", fnc_nam[..].to_string());
                lookup_tbl.push((loc.1 as u64, loc.2 as u64, n_msg));
                Backend::construct_expr(address, lookup_tbl, symtab,fnc_map, ns);
                for expp in args {
                    Backend::construct_expr(expp, lookup_tbl, symtab,fnc_map, ns);
                }

                Backend::construct_expr(value, lookup_tbl, symtab,fnc_map, ns);
            ...
            ExternalFunctionCallRaw {
                ...
                Backend::construct_expr(args, lookup_tbl, symtab,fnc_map, ns);
                Backend::construct_expr(address, lookup_tbl, symtab,fnc_map, ns);
                Backend::construct_expr(value, lookup_tbl, symtab,fnc_map, ns);
                Backend::construct_expr(gas, lookup_tbl, symtab,fnc_map, ns);
            }
            Constructor {
                ...
                if let Some(optval) = value {
                    Backend::construct_expr(optval, lookup_tbl, symtab,fnc_map, ns);
                }
                if let Some(optsalt) = salt {
                    Backend::construct_expr(optsalt, lookup_tbl, symtab,fnc_map, ns);
                }
            }
    ...
    fn construct_cont(
        ...
        if let Some(expr) = &contvar.initializer {
            Backend::construct_expr(&expr, lookup_tbl, samptb,fnc_map, ns);
        }
    }
    ...
    fn traverse(ns: &ast::Namespace, lookup_tbl: &mut Vec<(u64, u64, String)>, fnc_map: &mut HashMap<String, String>) {
        ...
        for evntdcl in &ns.events {
            let tag_msg = format!("{}", render(&evntdcl.tags[..]));
        ...
        for enm in &ns.enums {
            let tag_msg = format!("{}", render(&enm.tags[..]));

            let mut evnt_msg = format!("{} enum {} `{{`", tag_msg, enm.name);

            for (nam, vals) in &enm.values {
                evnt_msg = format!("{} {} {},", evnt_msg, nam, vals.1);
            }

            evnt_msg = format!("{} `}}`", evnt_msg);
            let ky = format!("{}", enm.name);
            fnc_map.insert(ky, evnt_msg);

            let msg_tg = render(&enm.tags[..]);
            lookup_tbl.push((enm.loc.1 as u64 , (enm.loc.1 + enm.name.len()) as u64, msg_tg));
        }

        for strct in &ns.structs {
                let tag_msg = format!("{}", render(&strct.tags[..]));
    
                let mut temp_tbl: Vec<(u64, u64, String)> = Vec::new();
                let mut evnt_msg = format!("{} struct {} `{{`", tag_msg, strct.name);
            ...
            lookup_tbl.push((strct.loc.1 as u64 , (strct.loc.1 + strct.name.len()) as u64, msg_tg));
        }

        for contrct in &ns.contracts {
            let msg_tg = render(&contrct.tags[..]);
            lookup_tbl.push((contrct.loc.1 as u64, (contrct.loc.1 + msg_tg.len()) as u64, msg_tg));
            ...
            for fnc in &contrct.functions {
                ...
                for parm in &fnc.params {
                    match parm.ty {
                            Type::Struct(no) => {
                                let fnc_ky = format!("{}", ns.structs[no].name);

                                if fnc_map.contains_key(&fnc_ky) {
                                    match fnc_map.get(&fnc_ky) {
                                        Some(ans) => {
                                                let nm = format!("{}", ans[..].to_string());
                                                lookup_tbl.push((parm.loc.1 as u64, (parm.loc.2 /*+ fnc_ky.len() + 7*/) as u64, nm));
                                            }
                                            _ => {
                                                let nm = format!("unable to parse {}", fnc_ky);
                                                lookup_tbl.push((parm.loc.1 as u64, (parm.loc.2 /*+ fnc_ky.len() + 7*/) as u64, nm));
                                            }
                                        }
                                    }
                            ...
                            Type::Enum(no) => {
                                let fnc_ky = format!("{}", ns.enums[no].name);
        
                                if fnc_map.contains_key(&fnc_ky) {
                                    match fnc_map.get(&fnc_ky) {
                                        Some(ans) => {
                                                let nm = format!("{}", ans[..].to_string());
                                                lookup_tbl.push((parm.loc.1 as u64, (parm.loc.2 /*+ fnc_ky.len() + 5*/) as u64, nm));
                                            }
                                            _ => {
                                                let nm = format!("unable to parse {}", fnc_ky);
                                                lookup_tbl.push((parm.loc.1 as u64, (parm.loc.2 /*+ fnc_ky.len() + 7*/) as u64, nm));
            ...
            for varscont in &contrct.variables {
                ...
                Backend::construct_cont(varscont, lookup_tbl, &samptb,fnc_map, ns);

                let msg_tg = render(&varscont.tags[..]);
                lookup_tbl.push((varscont.loc.1 as u64, (varscont.loc.1 + varscont.name.len()) as u64, msg_tg));
            }
        }
        ...
        for entdcl in &ns.events {
            ...
            lookup_tbl.push((entdcl.loc.1 as u64 ,(entdcl.loc.1 + entdcl.name.len()) as u64, msg_tg));
        }
    }
    ...
    fn get_hover_msg(offset: &u64, lookup_tbl: &[(u64, u64, String)], _fnc_map: &HashMap<String, String>) -> String {
        let mut res = format!(
            "Either the code is incorrect or Feature not yet implemented for {} offset",
            offset
        );
        for (_l1, _r1, msg) in lookup_tbl {

            
            if *_l1 <= *offset && *offset <= *_r1 {
                let new_msg = &msg[..];
                res = format!("{}", new_msg.to_string());
            ...
            /*
            let mut msg1 = format!("AAA {} {} {} BBB\n", _l1, _r1, msg);
            msg1 = msg1.to_owned();
            res.push_str(&msg1);
            */
        }
        /*
        let mut samp = format!(" end start");
        samp = samp.to_owned();
        res.push_str(&samp);
        for fnc in _fnc_map {
            let mut samp1 = format!("CCC {} {} DDD", fnc.0, fnc.1);
...
impl LanguageServer for Backend {
    ...
    async fn hover(&self, hverparam: HoverParams) -> Result<Option<Hover>> {
        ...
        if let Ok(path) = uri.to_file_path() {
            ...
            Backend::traverse(&ns, &mut lookup_tbl, &mut fnc_map);
            ...
            let msg = Backend::get_hover_msg(&offst, &lookup_tbl, &fnc_map);
